...
// TODO: Re-enable once the corresponding backend changes are deployed to the CI environment.
// These tests rely on ui.rest.apitoken.generator.enable and
// ui.rest.api.write.access.token.in.preferences.enabled being supported by the backend.
test.describe.skip('REST API Token', () => {
    ...
    const setTokenGeneratorAndSave = async (page: Page, desiredState: boolean): Promise<void> => {
        ...
        await openFrontendConfigurations(page)
        const verifiedTokenGeneratorToggle = page.locator(tokenGeneratorToggleSelector)
        await expect(verifiedTokenGeneratorToggle).toHaveCount(1, { timeout: 10000 })
        if (desiredState) {
            await expect(verifiedTokenGeneratorToggle).toBeChecked()
        } else {
            await expect(verifiedTokenGeneratorToggle).not.toBeChecked()
        }
    }
    ...
    const setWriteAccessAndSave = async (page: Page, desiredState: boolean): Promise<void> => {
        ...
        await page.reload({ waitUntil: 'domcontentloaded' })
        await openFrontendConfigurations(page)
        const verifiedWriteAccessToggle = page.locator(writeAccessToggleSelector)
        await expect(verifiedWriteAccessToggle).toHaveCount(1, { timeout: 10000 })
        if (desiredState) {
            await expect(verifiedWriteAccessToggle).toBeChecked()
        } else {
            await expect(verifiedWriteAccessToggle).not.toBeChecked()
        }
    }
