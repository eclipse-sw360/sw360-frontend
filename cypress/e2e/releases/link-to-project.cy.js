...
const verifySuccessMessage = (testData) => {
    cy.get(viewSelectors.linkToPJSuccessMessage).contains(`The release ${testData.release.name}(${testData.release.version}) has been successfully linked to project ${testData.linkedProject.name}`)
    cy.get(viewSelectors.linkToPJSuccessMessage).contains('Click here to edit the release relation as well as the project mainline state in the project')
}
...
const openLinkModal = (release, isComponentPage) => {
    ...
    if (isComponentPage) {
        ...
        cy.get(viewSelectors.btnLinkToProject).click()
    }
}

const selectProjectToLink = (linkedProject) => {
    cy.get(viewSelectors.dialogLinkReleaseToProject.btnSearch).click()
    ...
    cy.get(viewSelectors.dialogLinkReleaseToProject.btnsCloseAndLinkToProject).last().click()
}
