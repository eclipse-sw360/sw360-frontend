...
export default function Attachments({
    ...
    useEffect(() => {
        ...
        const targetSourceAttachmentFileIds =
            targetAttachments?.filter((a) => a.attachmentType === 'SOURCE').map((a) => `${a.sha1}`) ?? []

        const sourceSourceAttachmentFileIds =
            sourceAttachments?.filter((a) => a.attachmentType === 'SOURCE').map((a) => `${a.sha1}`) ?? []
        ...
        const targetOtherAttachmentFileIds =
            targetAttachments?.filter((a) => a.attachmentType !== 'SOURCE').map((a) => `${a.sha1}`) ?? []

        const sourceOtherAttachmentFileIds =
            sourceAttachments?.filter((a) => a.attachmentType !== 'SOURCE').map((a) => `${a.sha1}`) ?? []
        ...
        const otherTargetAttachmentsResult = [
            ...
            ...targetOtherAttachmentFileIds.map((id) => ({
                value: id,
    ...
    useEffect(() => {
        ...
        sourceReleaseDetail,
        matchingSourceAttachmentMergeList,
    ])

    return (
        <>
            {targetRelease && sourceReleaseDetail && finalReleasePayload && targetAttachments && sourceAttachments && (
                <div className='mb-3'>
                                            ...
                                            <div className='mt-2 col text-end'>
                                                {`${
                                                    targetAttachments.find(
                                                        (attachmentData) => attachmentData.sha1 === data.value,
                                                    )?.filename
                                                }`}{' '}
                                                {`(${t('SOURCE')})`}
                                            </div>
