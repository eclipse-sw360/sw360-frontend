    ...
    <table className={`table label-value-table ${styles['summary-table']}`}>
      ...
      <tbody hidden={toggle}>
        ...
        <tr>
          ...
          <td>
            {(release['_embedded'] && !CommonUtils.isNullEmptyOrUndefinedArray(release['_embedded']['sw360:contributors'])) && (
        ...
        <tr>
          <td>{t('Moderators')}:</td>
          <td>
            {(release['_embedded'] && !CommonUtils.isNullEmptyOrUndefinedArray(release['_embedded']['sw360:moderators'])) && (
        ...
        <tr>
          <td>{t('Subscribers')}:</td>
          <td>
            {(release['_embedded'] && !CommonUtils.isNullEmptyOrUndefinedArray(release['_embedded']['sw360:subscribers'])) && (
        ...
        <tr>
          ...
          <td>
            {(!CommonUtils.isNullEmptyOrUndefinedArray(release.roles)) && (
              Object.entries(release.roles)
                .map(([key, value]: any) => (
                  <li key={key}>
                    <b>{key}: </b>
                    {Object.entries(value).map(([index, item]: any) => (
        ...
        <tr>
          ...
          <td>
           {(release['_embedded'] && !CommonUtils.isNullEmptyOrUndefinedArray(release['_embedded']['sw360:licenses'])) &&  (
                Object.entries(release['_embedded']['sw360:licenses'])
                    .map(([index, item]: any) => (
                      <span key={index}  >
                        {item.shortName}
                        <FaInfoCircle style={{marginLeft: '5px', color: 'gray'}} className={styles.info}/>
                      </span>
                    ))
                    .reduce((prev, curr): any => [prev, <>, <br /></>, curr])
              )}
          </td>
        </tr>
        <tr>
        ...
        <tr>
          ...
          <td>
            {(release['externalIds']) && <ExternalIds externalIds={release['externalIds']} />}
          </td>
        </tr>
        <tr>
