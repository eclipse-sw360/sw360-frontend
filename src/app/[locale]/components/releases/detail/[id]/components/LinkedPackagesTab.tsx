...
import { OverlayTrigger, Spinner, Tooltip, Modal, Alert } from 'react-bootstrap'
        ...
        try {
            ...
            if (response.status === StatusCodes.OK || response.status === StatusCodes.NO_CONTENT) {
                ...
                setAlert({
                    ...
                    message: (
                        <>
                            {t('The Package cannot be deleted, Since it is used by other project Please unlink it before deleting')}
                        </>
                    ),
                })
            } else {
