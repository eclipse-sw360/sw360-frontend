    ...
    try {
      if (response.status == HttpStatus.MULTIPLE_STATUS) {
        ...
        if (deleteStatus == HttpStatus.OK) {
          ...
          displayMessage('danger', 'I could not delete the release, since it is used by another component (release) or project')
  ...
  return (
    ...
    >
        ...
        <Form>
          ...
          {(release['_embedded'] && release['_embedded']['sw360:attachments']) &&
            <>
              ...
              <ul>
                <li>
                  {`${t('Attachments')}: ${release['_embedded']['sw360:attachments'].length}`}
                </li>
              </ul>
            </>}
          <hr />
      ...
      <Modal.Footer className='justify-content-end' >
        ...
        <Button className='login-btn' variant='danger' onClick={handleSubmit} hidden={reloadPage}>{t('Delete Release')}</Button>
      </Modal.Footer>
    </Modal>
  )
}
