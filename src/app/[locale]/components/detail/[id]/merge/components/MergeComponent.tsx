    ...
    useEffect(() => {
        ...
        setExternalIdsMergeList([
            ...Object.keys(targetComponent?.externalIds ?? {})
                .filter(
                    (targetElem) =>
                        Object.keys(sourceComponent?.externalIds ?? {}).filter(
            ...
            ...Object.keys(sourceComponent?.externalIds ?? {})
                .filter(
                    (sourceElem) =>
                        Object.keys(targetComponent?.externalIds ?? {}).filter(
        ...
        setAdditionalDataMergeList([
            ...Object.keys(targetComponent?.additionalData ?? {})
                .filter(
                    (targetElem) =>
                        Object.keys(sourceComponent?.additionalData ?? {}).filter(
                            (sourceElem) => sourceElem === targetElem,
                        ).length !== 0,
                )
                .map((c) => ({
            ...
            ...Object.keys(sourceComponent?.additionalData ?? {})
                .filter(
                    (sourceElem) =>
                        Object.keys(targetComponent?.additionalData ?? {}).filter(
                            (targetElem) => sourceElem === targetElem,
                        ).length === 0,
                )
                .map((c) => ({
        ...
        setAttachmentsMergeList([
            ...(targetComponent?._embedded?.['sw360:attachments'] ?? []).map((c) => ({
                value: c.attachmentContentId ?? '',
                presentInSource: false,
                presentInTarget: true,
                overWritten: false,
            })),

            ...(sourceComponent?._embedded?.['sw360:attachments'] ?? []).map((c) => ({
                value: c.attachmentContentId ?? '',
                presentInSource: true,
                presentInTarget: false,
                overWritten: false,
            ...
            for (const role in sourceComponent.roles) {
                ...
                mergeLists[role] = [
                    ...
                    ...sourceComponent.roles[role]
                        .filter(
                            (sourceElem) =>
                                (targetComponent?.roles?.[role] ?? []).filter((targetElem) => sourceElem === targetElem)
                                    .length === 0,
                        )
                        .map((elem) => ({
                            value: elem,
                                ...
                                {id.presentInSource && id.presentInTarget && (
                                    <div className='d-flex row'>
                                        ...
                                        <div className='mt-2 col text-start'>
                                            {sourceComponent.externalIds?.[id.value]}
                                        </div>
                                    </div>
                                )}
                                {!id.presentInSource && id.presentInTarget && (
                                    <div className='d-flex row'>
                                        <div className='mt-2 col text-end'>
                                            {finalComponentPayload.externalIds?.[id.value]}
                                        </div>
                                        <div className='col-12 col-md-2 mx-5 text-center'>
                                            {finalComponentPayload.externalIds?.[id.value] ===
                                            targetComponent.externalIds?.[id.value] ? (
                            ...
                            >
                                ...
                                {id.presentInSource && id.presentInTarget && (
                                    <div className='d-flex row'>
                                        ...
                                        <div className='mt-2 col text-start'>
                                            {sourceComponent.additionalData?.[id.value]}
                                        </div>
                                    </div>
                                )}
                                {!id.presentInSource && id.presentInTarget && (
                                    <div className='d-flex row'>
                                        <div className='mt-2 col text-end'>
                                            {finalComponentPayload.additionalData?.[id.value]}
                                        </div>
                                        <div className='col-12 col-md-2 mx-5 text-center'>
                                            {finalComponentPayload.additionalData?.[id.value] ===
                                            targetComponent.additionalData?.[id.value] ? (
                                    ...
                                    {assignees.map((assignee) => {
                                        if (assignee.presentInSource && assignee.presentInTarget) {
                                            ...
                                            return (
                                                ...
                                                >
                                                    <div className='mt-2 col text-end'>
                                                        {assignee.overWritten ? '' : assignee.value}
                                                    </div>
                                                    <div className='col-12 col-md-2 mx-5 text-center'>
                                                        {!assignee.overWritten ? (
                                                            <button
                                                                ...
                                                                onClick={() => {
                                                                    const newAssignees = (
                                                                        finalComponentPayload.roles?.[role] ?? []
                                                                    ).filter((a) => a !== assignee.value)
                                                                    setFinalComponentPayload({
                                                                        ...finalComponentPayload,
                                                                        roles: {
                                                                    ...
                                                                    setAdditionalRolesMergeLists({
                                                                        ...additionalRolesMergeLists,
                                                                        [role]: updatedList,
                                                                    })
                                                                }}
                                                            >
                        ...
                        {attachmentsMergeList.map((c, i) => {
                            if (c.presentInTarget && !c.presentInSource) {
                                const att = targetComponent._embedded?.['sw360:attachments']?.filter(
                                    (a) => a.attachmentContentId === c.value,
                                )[0]
                                return (
                                    <div
                                        className={`border border-blue p-2 ${i !== 0 ? 'border-top-0' : ''}`}
                                            ...
                                            <div className='col-12 col-md-2 mx-5 text-center'>
                                                {!c.overWritten ? (
                                                    <button
                                                        ...
                                                        onClick={() => {
                                                            const attachments = (
                                                                finalComponentPayload.attachments ?? []
                                                            ).filter((a) => a.attachmentContentId !== c.value)
                                                            setFinalComponentPayload({
                                                                ...finalComponentPayload,
                                                                attachments,
                                                            })

                                                            const updatedAttachmentsMergeList =
                                                                attachmentsMergeList.map((a) => {
                                                                    if (a.value === c.value) {
                                                                        return {
                                                                            ...a,
                                                                            overWritten: true,
