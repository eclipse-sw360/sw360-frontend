...
/**
 * Returns `true` if user has VIEWER role (read-only access).
 */
const isViewer = (user: SessionUser | undefined | null): boolean => user?.userGroup === UserGroupType.VIEWER
...
const getStandardPermissions = (
    ...
    if (isViewer(user) && action !== RequestedAction.READ) {
        return false
    }
...
/**
 * Checks if user can perform write operations (not VIEWER).
 * Quick check without record-level context.
 */
const canWrite = (user: SessionUser | undefined | null): boolean => {
