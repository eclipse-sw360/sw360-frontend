...
const ComponentVulnerabilities = ({ vulnerData }: Props) => {
  ...
  const columns = [
    ...
    {
      name: t('External Id'),
      formatter: ([externalId, id]: Array<string>) => _(<Link href={'/vulnerabilites/detail/' + id} className='link'>{externalId}</Link>),
      sort: true
    },
    {
      name: t('Priority'),
  ...
  const data = Object.entries(vulnerData).map(([index, item]: any) => [
    item.externalId, item.intReleaseName,
    [item.externalId, item.releaseVulnerabilityRelation.vulnerabilityId],
    item.priority, item.releaseVulnerabilityRelation.matchedBy,
    item.title, t(item.releaseVulnerabilityRelation.verificationStateInfo.at(-1).verificationState), item.projectAction
  ])
  ...
  return (
    <div className='row'>
      ...
      <div className='col-12'>
        <h5 style={{ color: '#5D8EA9', borderBottom: '1px solid #5D8EA9', fontSize: '1rem', fontWeight: 'bold' }}>VULNERABILITIES</h5>
        <SW360Table
          columns={columns}
          data={data}
        ...
        <Form.Group className='mb-3' controlId='createdOn'>
          <Form.Label><b>{t('Change verification state of selected vulnerabilities to')}</b></Form.Label>
          <Form.Select size='sm'
            onChange={handleChangeState}
            style={{
              display: 'inline-block', width: '170px', marginLeft: '0.75rem',
              padding: '8px', marginRight: '0.75rem', fontWeight: '500'
            }}>
            {Object.values(VulnerabilitiesVerificationState).map((item): JSX.Element =>
              <option key={item} value={item}> {t(item)} </option>
            )}
          </Form.Select>
          <Button onClick={handleClick}>{t('Change State')}</Button>
        </Form.Group>
        <h5 style={{ color: '#5D8EA9', borderBottom: '1px solid #5D8EA9', fontSize: '1rem', fontWeight: 'bold' }}>VULNERABILITY MATCHING STATISTICS</h5>
      </div>
